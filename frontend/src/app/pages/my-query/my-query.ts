...
export class MyQuery {
  ...
  constructor(private readonly api: FoodEntryApi) { this.search(); }
  ...
  search(page = 1): void {
    ...
    this.api.list({
      ...
      error: () => { this.loading.set(false); this.message.set('We could not load those entries.'); },
    });
  }

  reset(): void {
    this.form.reset({ from: this.daysAgo(30), to: this.today(), mealType: '', minCalories: '', maxCalories: '' });
    this.search();
  }

  edit(entry: FoodEntry): void { this.editingEntry.set(entry); this.showEditor.set(true); }
  remove(entry: FoodEntry): void {
    if (!window.confirm(`Delete ${entry.foodName}?`)) return;
    this.api.remove(entry.id).subscribe({ next: () => this.search(this.page()), error: () => this.message.set('We could not remove that entry.') });
  }
  saved(): void { this.showEditor.set(false); this.search(this.page()); }
  pageNumbers(): number[] { return Array.from({ length: this.totalPages() }, (_, index) => index + 1); }
  private today(): string { return new Date().toISOString().slice(0, 10); }
  private daysAgo(days: number): string { return new Date(Date.now() - days * 86400000).toISOString().slice(0, 10); }
