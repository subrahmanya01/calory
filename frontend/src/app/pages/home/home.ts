...
@Component({
  selector: 'app-home',
  imports: [CommonModule, ReactiveFormsModule, RouterLink, Header, Footer, Spinner, GoalEditor, FoodEntryEditor, ImageFoodReview, FoodPdfImport, Chat],
...
export class Home {
  ...
  get activeGoal(): HealthGoal | null {
    ...
    return this.goals().find((goal) => goal.startDate <= today && (!goal.endDate || goal.endDate >= today)) ?? null;
  }
  ...
  get averageCalories(): number { const points = this.trends(); return points.length ? Math.round(points.reduce((sum, point) => sum + point.calories, 0) / points.length) : 0; }
  get highestCalorieDay(): TrendPoint | null { return this.trends().reduce<TrendPoint | null>((highest, point) => !highest || point.calories > highest.calories ? point : highest, null); }
  get calorieLinePoints(): string {
    const points = this.trends();
    if (!points.length) return '';
    const width = points.length === 1 ? 50 : 100 / (points.length - 1);
    return points.map((point, index) => `${points.length === 1 ? 50 : index * width},${96 - (point.calories / this.trendPeak) * 82}`).join(' ');
  }
  get macroMetrics() { return [
  ...
  metricValue(point: TrendPoint, key: string): number { return Number(point[key as keyof TrendPoint]) || 0; }
  metricPeak(key: string): number { return Math.max(...this.trends().map((point) => this.metricValue(point, key)), 1); }
  metricLinePoints(key: string): string {
    const points = this.trends();
    if (!points.length) return '';
    const width = points.length === 1 ? 50 : 100 / (points.length - 1);
    const peak = this.metricPeak(key);
    return points.map((point, index) => `${points.length === 1 ? 50 : index * width},${96 - (this.metricValue(point, key) / peak) * 82}`).join(' ');
  }
  ...
  onPdfImported(result: ImportFoodEntriesResponse): void {
    ...
    this.message.set(`${result.importedCount} entr${result.importedCount === 1 ? 'y' : 'ies'} imported${result.skippedCount ? `, ${result.skippedCount} row${result.skippedCount === 1 ? '' : 's'} skipped.` : '.'}`);
    this.loadHealthData();
  }
  closeChat(): void { this.chatOpen.set(false); this.chatExpanded.set(false); }

  private loadHealthData(): void {
    const from = new Date(Date.now() - 6 * 86400000).toISOString().slice(0, 10);
    const to = this.today();
    this.goalApi.getAll().subscribe({ next: (goals) => this.goals.set(goals.items) });
    this.foodApi.list({ from, to, pageSize: 20 }).subscribe({ next: (entries) => this.entries.set(entries.items) });
